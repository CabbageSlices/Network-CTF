    ...
    for(int yPeicesCreated = 0; yPeicesCreated < yPeices; ++yPeicesCreated) {
        ...
        for(int xPeicesCreated = 0; xPeicesCreated < xPeices; ++xPeicesCreated) {
            ...
            if(newTexture->loadFromImage(image, area)) {
                ...
                //position of the sprite is the same as the position of the viewing area of the texture since thats where the texture peice is located on the image itself
                newSprite->setPosition(area.left, area.top);
