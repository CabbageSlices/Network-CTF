...
const sf::Color BUTTON_DOT = sf::Color(174, 29, 113, 255);
const sf::Color REPLACEMENT_COLOR = sf::Color(173, 28, 112, 254);
...
sf::Vector2f removeDot(sf::Image& image, const unsigned& startingXPosition, const unsigned& startingYPosition) {
    ...
    //determine width
    for(unsigned xPos = startingXPosition; (xPos < image.getSize().x) && image.getPixel(xPos, startingYPosition) == BUTTON_DOT; ++xPos) {
    ...
    //determine height
    for(unsigned yPos = startingYPosition; yPos < image.getSize().y && image.getPixel(startingXPosition, yPos) == BUTTON_DOT; ++yPos) {
